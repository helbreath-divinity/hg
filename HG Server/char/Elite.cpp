...
CElite::CElite(int NpcH) : CNpc(NpcH)
{
	_minions = 0;
	_visTargets = 0;
	...
	case NPC_SOR:
		...
		{			
				...
				time(0) - _timers[0] > 10)
			{
				...
				magicHandler(m_sX, m_sY+7, MAGIC_EARTHSHOCKWAVE); 
				magicHandler(m_sX+9, m_sY+7, MAGIC_EARTHSHOCKWAVE); 
				g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, m_sX + _tmp_cTmpDirX[m_cDir], m_sY + _tmp_cTmpDirY[m_cDir], 1); 
...
bool CElite::_SummonMinion()
{
	...
	CNpc * minion = g_game->CreateNpc(cNpcName, m_cMapIndex, 9, 
		MOVETYPE_RANDOM, &x, &y, m_side, cNpcWaypoint, NULL, NULL);
