...
bool checkResistingIceSuccess(char cAttackerDir, short sTargetH, char cTargetType, int iHitRatio)
{
	...
	case OWNERTYPE_NPC:
		if (g_npcList[sTargetH] == NULL) return false;
		if (g_npcList[sTargetH]->m_element == ELEMENT_WATER) return true; 
...
int calculateAttackEffect(short sTargetH, char cTargetType, short sAttackerH, char cAttackerType, int tdX, int tdY, int iAttackMode, bool bNearAttack, bool bIsDash)
{
	...
	case OWNERTYPE_PLAYER:
		...
		if(g_clientList[sAttackerH]->GetParty())
			iPartyID = g_clientList[sAttackerH]->GetParty()->GetID();
...
CAE_SKIPDAMAGEMOVE:;
							...
							if (dice(1,10000) >= iProb) 
								g_game->SendEventToNearClient_TypeA(sTargetH, OWNERTYPE_PLAYER, MSGID_MOTION_DAMAGE, iAP_SM, sAttackerWeapon, NULL);
						}
...
CAE_SKIPCOUNTERATTACK:;
				...
				if ((wWeaponType < 40) && (g_npcList[sTargetH]->m_cActionLimit == 4)) {
					...
					g_game->SendEventToNearClient_TypeA(sTargetH, OWNERTYPE_NPC, MSGID_MOTION_MOVE, NULL, NULL, NULL);
...
CAE_SKIPDAMAGEMOVE2:;
					...
					g_game->SendEventToNearClient_TypeA(sTargetH, OWNERTYPE_NPC, MSGID_MOTION_DAMAGE, iDamage, sAttackerWeapon, NULL);
				}
