...
void CClient::KilledHandler(int iAttackerH, char cAttackerType, short sDamage)
{
	...
	g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_PLAYER, MSGID_MOTION_DYING, sDamage, sAttackerWeapon, NULL);
...
void CClient::DecPKCount()
{
	if (m_iPKCount > 0) {
		...
		{	
			...
			g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_PLAYER, MSGID_MOTION_NULL, NULL, NULL, NULL);
		}
...
void CClient::Notify(int iFromH, uint16 wMsgType, uint32 sV1, uint32 sV2, uint32 sV3, char * pString, 
...
uint32 CClient::HasItem(char * name) const
{
	char cTmpName[21];
...
uint32 CClient::HasItem(ItemID id) const
{
	for (int i = 0; i < MAXITEMS; i++)
	{
		if (m_pItemList[i] && 
