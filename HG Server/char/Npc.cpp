	...
	for (i = 0; i < MAXNPCTYPES; i++) 
	{
		if (g_npcConfigList[i] != NULL) {
			if (memcmp(cTmpName, g_npcConfigList[i]->m_cNpcName, 20) == 0) {
				...
				m_element		   = g_npcConfigList[i]->m_element;
...
void CNpc::Behavior_Move()
{
	...
	if (m_cMoveType == MOVETYPE_FOLLOW) {
		...
		if (sDistance >= 3) {
			short DOType = 0;
			cDir = g_game->cGetNextMoveDir(sX, sY, dX, dY, m_cMapIndex, m_cTurn, &m_tmp_iError, &DOType);
			...
			if (cDir == 0) {
				...
				if(DOType == DYNAMICOBJECT_SPIKE)
				{
					...
					if(IsDead())
					{
						...
						g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_DAMAGE, dmg, 1, NULL);
					}
				}
				...
				m_sY   = dY;
				m_cDir = cDir;
				g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_MOVE, NULL, NULL, NULL);
			}
		}
	}
	else 
	{
		short DOType = 0;
		cDir = g_game->cGetNextMoveDir(m_sX, m_sY, m_dX, m_dY, 
	...
	case 5:
		...
		case 38: // Mana Collector
			if (m_sBehaviorTurnCount >= 3) {
				...
				if (bFlag == true) {
					g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, m_sX, m_sY, 1); 
				}
			}
			break;
	...
	if ((abs(sX - dX) <= 1) && (abs(sY - dY) <= 1)) {
		...
		if (m_cActionLimit == 5) {
			...
			case 89: // AGT
				if (target) { 
				if(m_cTargetType == OWNERTYPE_PLAYER) {
				g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, dX, dY, 1); 
			...
			case 87: // CT
				if (target) { 
				if(m_cTargetType == OWNERTYPE_PLAYER) {
				g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, m_sX + _tmp_cTmpDirX[cDir], m_sY + _tmp_cTmpDirY[cDir], 2); 				
			...
			{
				g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, m_sX + _tmp_cTmpDirX[cDir], m_sY + _tmp_cTmpDirY[cDir], 1); 
		...
		if ((m_iAttackRange > 1) && 
			(abs(sX - dX) <= m_iAttackRange) && (abs(sY - dY) <= m_iAttackRange)) {
				...
				case 87: // CT
						if (target) { 
						if(m_cTargetType == OWNERTYPE_PLAYER) {
						g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, dX, dY, 2); 
						calculateAttackEffect(m_iTargetIndex, m_cTargetType, m_handle, OWNERTYPE_NPC, dX, dY, 2);
							}
						}
						break;
					...
					case 79: 
						if (target)
						{
							...
							g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, dX, dY, 20); 
							calculateAttackEffect(m_iTargetIndex, m_cTargetType, m_handle, OWNERTYPE_NPC, dX, dY, 1);
						}
						break;
...
void CNpc::magicHandler(short dX, short dY, short sType) // magicHandler
{
	...
	iWeatherBonus = g_game->iGetWeatherMagicBonusEffect(spell, g_mapList[m_cMapIndex]->m_weather);
	...
	if (spell->m_dwDelayTime == 0) {
		...
		case MAGICTYPE_DAMAGE_LINEAR:
			...
			for (i = 2; i < 10; i++) {
				...
				for(int pNo=0; pNo < 5; pNo++){
					...
					if (CheckResistingMagicSuccess(m_cDir, sOwnerH, cOwnerType, iResult) == false)
						g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element);
					...
					if ( (cOwnerType == OWNERTYPE_PLAYER) && (g_clientList[sOwnerH] != NULL) &&
						(g_clientList[sOwnerH]->m_iHP > 0) ) {
							if (CheckResistingMagicSuccess(m_cDir, sOwnerH, cOwnerType, iResult) == false)
								g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARRANGE] + iWeatherBonus, false, spell->m_element);
					}
				}
			...
			g_mapList[m_cMapIndex]->GetOwner(&sOwnerH, &cOwnerType, dX, dY);
			if (CheckResistingMagicSuccess(m_cDir, sOwnerH, cOwnerType, iResult) == false)
				g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, false, spell->m_element);
		...
		case MAGICTYPE_ICE_LINEAR:
			...
			for (i = 2; i < 10; i++) {
				...
				for(int pNo=0; pNo < 5; pNo++){
					...
					if (target && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
						g_game->Effect_Damage_Spot_DamageMove(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, sX, sY, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element);
					...
					if (target && !target->IsDead() && target->IsPlayer() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
						g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element);
						if(target && !target->IsDead() && checkResistingIceSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false)
							target->AddMagicEffect(spell->m_sType, spell->m_sValue[MAGICV_ICEDURATION]);
					}
				}
			...
			if (target && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
				g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element); 
				if(target && !target->IsDead() && checkResistingIceSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false)
					target->AddMagicEffect(spell->m_sType, spell->m_sValue[MAGICV_ICEDURATION]);
			}
		...
		case MAGICTYPE_DAMAGE_SPOT:
			g_mapList[m_cMapIndex]->GetOwner(&sOwnerH, &cOwnerType, dX, dY);
			if (CheckResistingMagicSuccess(m_cDir, sOwnerH, cOwnerType, iResult) == false)
				g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element);
		...
		case MAGICTYPE_DAMAGE_AREA:
			...
			for (iy = dY - spell->m_vRange; iy <= dY + spell->m_vRange; iy++)
				for (ix = dX - spell->m_hRange; ix <= dX + spell->m_hRange; ix++) {
					g_mapList[m_cMapIndex]->GetOwner(&sOwnerH, &cOwnerType, ix, iy);
					if (CheckResistingMagicSuccess(m_cDir, sOwnerH, cOwnerType, iResult) == false)
						g_game->Effect_Damage_Spot_DamageMove(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, dX, dY, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element);
		...
		case MAGICTYPE_DAMAGE_AREA_NOSPOT_SPDOWN:
			for (iy = dY - spell->m_vRange; iy <= dY + spell->m_vRange; iy++)
			{
				for (ix = dX - spell->m_hRange; ix <= dX + spell->m_hRange; ix++)
				{
					g_mapList[m_cMapIndex]->GetOwner(&sOwnerH, &cOwnerType, ix, iy);
					if (CheckResistingMagicSuccess(m_cDir, sOwnerH, cOwnerType, iResult) == false) {
						g_game->Effect_Damage_Spot_DamageMove(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, dX, dY, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, false, spell->m_element, spell);
						g_game->Effect_SpDown_Spot(m_handle, OWNERTYPE_NPC, sOwnerH, cOwnerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS]);
					}
				...
				for (ix = dX - spell->m_hRange; ix <= dX + spell->m_hRange; ix++) {
					...
					if(target && !target->IsDead() && ((target->IsPlayer() && !target->IsNeutral()) || 
						...
						if ((CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) 
							...
							g_game->Effect_Damage_Spot_DamageMove(m_handle, OWNERTYPE_NPC,  target->m_handle, target->m_ownerType, dX, dY, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element);
			...
			for (i = 2; i < 10; i++) {
				...
				for(int pNo = 0; pNo < 5; pNo++){
					...
					if (target && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
						...
						g_game->Effect_Damage_Spot_DamageMove(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, sX, sY, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element, spell);
						if (target && !target->IsDead() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
							g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element, spell);
					...
					if (target && !target->IsDead() && target->IsPlayer() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false){
						...
						g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element, spell);
						if (target && !target->IsDead() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
							g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element, spell);
							g_game->Effect_SpDown_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS]);
						}
			...
			for (iy = dY - spell->m_vRange; iy <= dY + spell->m_vRange; iy++)
				for (ix = dX - spell->m_hRange; ix <= dX + spell->m_hRange; ix++) {
					...
					if (target && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
						...
						g_game->Effect_Damage_Spot_DamageMove(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, dX, dY, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS] + iWeatherBonus, false, spell->m_element, spell);
						if (target && !target->IsDead() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
							g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element, spell);
							g_game->Effect_SpDown_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS]);
						}
				...
				if (target && !target->IsDead() && target->IsPlayer() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false){
					...
					g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, false, spell->m_element, spell);
					if (target && !target->IsDead() && CheckResistingMagicSuccess(m_cDir, target->m_handle, target->m_ownerType, iResult) == false) {
						g_game->Effect_Damage_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS] + iWeatherBonus, true, spell->m_element, spell);
						g_game->Effect_SpDown_Spot(m_handle, OWNERTYPE_NPC, target->m_handle, target->m_ownerType, spell->m_sValue[MAGICV_LINEARTHROW], spell->m_sValue[MAGICV_LINEARRANGE], spell->m_sValue[MAGICV_LINEARBONUS]);
					}
...
void CNpc::Behavior_Death(Unit * attacker, int16 dmg)
{
	...
	g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_DYING, dmg, sAttackerWeapon, NULL);
...
void CNpc::Cast(short x, short y, short spell)
{
	...
	g_game->SendEventToNearClient_TypeA(m_handle, OWNERTYPE_NPC, MSGID_MOTION_ATTACK, 
		x + _tmp_cTmpDirX[m_cDir], y + _tmp_cTmpDirY[m_cDir], 1); 
}


bool CNpc::Follow(Unit * master)
{
	if(!master || master->m_cMapIndex != m_cMapIndex)
		return false;

	m_cMoveType = MOVETYPE_FOLLOW;
	m_cFollowOwnerType = master->m_ownerType;
	m_iFollowOwnerIndex = master->m_handle;
	m_side = master->m_side;

	return true;
}

void CNpc::SetTarget(Unit * target, bool isperm)
{	
	m_cBehavior = BEHAVIOR_ATTACK;
	m_sBehaviorTurnCount = 0;
	m_iTargetIndex = target->m_handle;
	m_cTargetType  = target->m_ownerType;
