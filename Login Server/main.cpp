		...
		case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
				...
				case IDOK:
					...
					UINT iResult = 0;
					mysql_init(&mySQL);
					if(!mysql_real_connect(&mySQL, Server->mySqlAddress, Server->mySqlUser, Server->mySqlPwd, Server->mySqlDatabase.c_str(), Server->mySqlPort, NULL, NULL)){
